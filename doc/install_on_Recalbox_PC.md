...
1. Remove the init script (we need to autostart *dynquee* a different way on PC):
    ```sh
    mount -o rw,remount /
...
Type `xrandr` to get a list of available video outputs.
...
## Configure *dynquee*
Edit the *dynquee* config file [`dynquee.ini`](../dynquee.ini) and change the settings in the `[slideshow]` section as follows:
```ini
viewer = /usr/bin/mpv
...
If you have a video card that supports hardware acceleration you could add the [`mpv --hwdec` option][mpv--hwdec] to the `video_player_opts` setting e.g. `--hwdec nvdec` for nVidia GPUs or `--hwdec vaapi` for AMD or Intel GPUs.
...
To prevent this, edit the [Openbox][openbox] config file `/etc/openbox/rc.xml` and add the following lines after the line which reads `<applications>` (indentation is not important):
...
```
This tells Openbox to give Emulation Station the focus when it launches, but never to give `mpv` the focus.
